        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-4 py-2.5 text-left font-semibold text-slate-600">Marks Obtained</th>
              <th className="px-4 py-2.5 text-left font-semibold text-slate-600">Total Marks</th>
              <th className="px-4 py-2.5 text-right font-semibold text-slate-600">Action</th>
            </tr>
          </thead>
          ...
          return (
            <div key={student.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <button
                ...
                disabled={isSubmitting || !draft.marks || !draft.totalMarks}
                onClick={() => onSubmit(student.id, Number(draft.marks), Number(draft.totalMarks))}
                className="mt-2.5 min-h-10 w-full rounded-md bg-brand-600 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50"
              >
                Save
              </button>
            </div>
